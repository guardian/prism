...
class Route53ZoneCollectorSet(accounts: Accounts)
    extends CollectorSet[Route53Zone](
      ResourceType("route53Zones"),
      accounts,
      Some(Global)
    ) {
  ...
  val lookupCollector: PartialFunction[Origin, Collector[Route53Zone]] = {
    case amazon: AmazonOrigin =>
      Route53ZoneCollector(amazon, resource, amazon.crawlRate(resource.name))
  }
}

case class Route53ZoneCollector(
    origin: AmazonOrigin,
    resource: ResourceType,
    crawlRate: CrawlRate
) extends Collector[Route53Zone]
    with Logging {

  val client: Route53Client = Route53Client.builder
  ...
  def crawl: Iterable[Route53Zone] = {
    client
      .listHostedZonesPaginator(ListHostedZonesRequest.builder.build)
      .hostedZones
      .asScala
      .map { zone =>
        val records = client
          .listResourceRecordSetsPaginator(
            ListResourceRecordSetsRequest.builder.hostedZoneId(zone.id).build
          )
          .resourceRecordSets
          .asScala
...
case class Route53Alias(
    dnsName: String,
    hostedZoneId: String,
    evaluateHealth: Boolean
)

case class Route53Record(
    name: String,
    recordType: String,
    ttl: Option[Long],
    records: Option[List[String]],
    aliasTarget: Option[Route53Alias]
)
