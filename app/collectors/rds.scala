...
class RdsCollectorSet(accounts: Accounts)
    extends CollectorSet[Rds](ResourceType("rds"), accounts, Some(Regional)) {
  val lookupCollector: PartialFunction[Origin, Collector[Rds]] = {
    case amazon: AmazonOrigin =>
      AWSRdsCollector(amazon, resource, amazon.crawlRate(resource.name))
  }
}

case class AWSRdsCollector(
    origin: AmazonOrigin,
    resource: ResourceType,
    crawlRate: CrawlRate
) extends Collector[Rds]
    with Logging {

  val client: RdsClient = RdsClient.builder
  ...
  def crawl: Iterable[Rds] =
    client
      .describeDBInstancesPaginator(DescribeDbInstancesRequest.builder.build)
      .dbInstances
      .asScala
      .map(Rds.fromApiData(_))
}
...
object Rds {
  def fromApiData(db: DBInstance): Rds = Rds(
    ...
    vpcSecurityGroupId =
      db.vpcSecurityGroups.asScala.map(_.vpcSecurityGroupId).toList,
