...
class AppComponents(context: ApplicationLoader.Context)
  ...
  log.info(
    "Calling init() on Lifecycle singletons: %s" format lifecycleSingletons
  ...
  lifecycleSingletons foreach { singleton =>
    try {
      ...
      case t: Throwable =>
        log.error(
          "Caught unhandled exception whilst calling init() on Lifecycle singleton",
          t
        )
    }
  }
  ...
  context.lifecycle.addStopHook(() =>
    Future {
      log.info(
        "Calling shutdown() on Lifecycle singletons: %s" format lifecycleSingletons.reverse
          .map(_.getClass.getName)
          .mkString(", ")
      )
  ...
  lazy val homeController =
    new Application(controllerComponents, () => router.documentation)

  lazy val apiController =
