...
class Api(
  ...
  def sourceAccounts: Action[AnyContent] = Action.async { implicit request =>
    ApiResult.noSource {
      val accounts = prismDataStore.accounts.all
        .collect {
  ...
  def healthCheck = Action.async { implicit request =>
    ApiResult.filter {
      val sources = prismDataStore.sourceStatusAgent.sources
      val notInitialisedSources =
        sources.data.filter(_.state.status != "success")
      if (notInitialisedSources.isEmpty) Map.empty
      else Map(sources.label -> notInitialisedSources)
      ...
      if (notInitialisedSources.isEmpty) {
        ...
        throw ApiCallException(
          Json.obj(
            "healthcheck" -> "not yet initialised",
            "sources" -> notInitialisedSources.values.headOption
          ),
          SERVICE_UNAVAILABLE
        )
    }
  ...
  def launchConfigurationList = Action.async { implicit request =>
    Api.itemList(
      prismDataStore.launchConfigurationAgent,
